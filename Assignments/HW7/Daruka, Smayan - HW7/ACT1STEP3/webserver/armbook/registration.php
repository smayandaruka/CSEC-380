...
if($passedChecks !== True){
	...
	if($good === true){
		// We don't check the bool oh well
		$session_id = bin2hex(openssl_random_pseudo_bytes(22));
		$session_id = substr($session_id,0,22);
        if($stmt = $mysqli->prepare("INSERT INTO sessions (user_id,session_id) VALUES (?,?)")){
    	        if($stmt->bind_param("is",$id, $session_id )){
            		if(!$stmt->execute()){
                		die("Error - Issue executing prepared statement: " . mysqli_error($mysqli));
                	}
                }else{
